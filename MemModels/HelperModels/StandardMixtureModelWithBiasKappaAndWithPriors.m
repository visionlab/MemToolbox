...
function y = SMMjeffreys(p)
  
  % Jeffreys prior for a proportion
  y_g = JeffreysPriorForProportion(p(2));
  
  % Jeffreys prior for kappa parameter of von Mises
  y_K = JeffreysPriorForKappaOfVonMises(p(3));

  % Jeffreys Prior for mean of circular data
  y_mu = ImproperUniform(p(1));
