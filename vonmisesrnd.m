...
if(kappa == 0)
	thetas = reshape((rand(n,1)*2-1)*pi, dims);
	return;
end

% set up constants using mu and kappa
...
while(~all(pass));
	...
	f(~pass) = (1 + r.*cos(pi.*u(:,1)))./(r+cos(pi.*u(:,1)));
...
thetas = reshape(mod(mu + pi + sign(rand(n,1)-0.5).*acos(f),pi*2) - pi, dims);
